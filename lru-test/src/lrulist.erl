...
%% Left tree: MAC adress sorted
%% Right tree: Port sorted with exp time.
new() ->
...
insert(Key, Value, LRUL) ->	insert(Key, Value, LRUL, []).

%% @doc Insert a new value into the container.
%% Options are
%% 		{slidingexpire, exp_val} 	: Set to expire exp_val seconds after insertion.
%% 		{absoluteexpire, exp_time} 	: Set to expire at the exact time exp_time.
%% @end  
insert(Key, Value, {Tree, LRUTree}, Options) ->
	Now = calendar:datetime_to_gregorian_seconds({date(), time()}),
	Data0 = init_data(Options, Now,Value),
	NewTree = gb_trees:enter(Key, Data0, Tree),
	NewLRUTree = enter_lru(Key, Data0#data.expire, LRUTree),
...
#data{expire = calc_expire(Rules, Now), lastAccess = Now, rules = Rules, data=Value}.
...
% gb_trees:is_defined -> true if key is in tree, false otherwise
% insert/add {Exp,Key} as a new key:value to the LRUTree 
enter_lru(Key, ExpTime, LRUTree) ->
	case gb_trees:is_defined(ExpTime, LRUTree) of
		true ->
			gb_trees_update_fun(ExpTime, fun(List) -> [Key|List] end, LRUTree);
		false ->
			gb_trees:insert(ExpTime, [Key], LRUTree)
	end.
